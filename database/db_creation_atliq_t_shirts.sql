...
-- Create the t_shirts table with prices in Indian Rupees
CREATE TABLE t_shirts (
    t_shirt_id INT AUTO_INCREMENT PRIMARY KEY,
    brand ENUM('Van Heusen', 'Levi', 'Nike', 'Adidas') NOT NULL,
    color ENUM('Red', 'Blue', 'Black', 'White') NOT NULL,
    size ENUM('XS', 'S', 'M', 'L', 'XL') NOT NULL,
    price INT CHECK (price BETWEEN 800 AND 4000),  -- Price in Indian Rupees (₹800 to ₹4000)
...
BEGIN
    ...
    DECLARE brand ENUM('Van Heusen', 'Levi', 'Nike', 'Adidas');
    ...
    WHILE counter < max_records DO
        ...
        SET brand = ELT(FLOOR(1 + RAND() * 4), 'Van Heusen', 'Levi', 'Nike', 'Adidas');
        SET color = ELT(FLOOR(1 + RAND() * 4), 'Red', 'Blue', 'Black', 'White');
        SET size = ELT(FLOOR(1 + RAND() * 5), 'XS', 'S', 'M', 'L', 'XL');
        SET price = FLOOR(800 + RAND() * 3201);  -- Random price between ₹800 and ₹4000
